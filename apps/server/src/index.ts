/**
 * Socket.IO server entry point for Kolab.
 * Features:
...
import {
  ...
  ScrollServiceMsg,
  StreamServiceMsg,
} from "@Kolab/types/message";
import type { Cursor, EditOp } from "@Kolab/types/operation";
import type { Pointer } from "@Kolab/types/pointer";
import type { Scroll } from "@Kolab/types/scroll";
import type {
  ClientToServerEvents,
  ServerToClientEvents,
} from "@Kolab/types/socket-events";
import type { ExecutionResult } from "@Kolab/types/terminal";
...
app.listen(PORT, (token) => {
  ...
  console.log(`Kolab-server listening on port: ${PORT}`);
});
...
app.get("/", (res, req) => {
  ...
  res.end(
    "Hello from Kolab-server! Go to https://Kolab.dulapahv.dev/ to start coding."
  );
});
