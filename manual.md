...
- [Kolab User Manual](#Kolab-user-manual)
  - [Getting Started](#getting-started)
    - [Accessing Kolab](#accessing-Kolab)
...
Kolab is a web-based application that requires no installation. To access Kolab:

1. Open your web browser
2. Navigate to [https://Kolab.dulapahv.dev/](https://Kolab.dulapahv.dev/).
...
Kolab allows you to share your room with others using multiple methods:
...
The Monaco Editor forms the core of Kolab's collaborative coding environment, enabling multiple users to code together in real-time. Key features include:
...
File name will be `Kolab-terminal-<date>--<time>.txt`, for example `Kolab-terminal-06-01-2025--17-37-28.txt`

- Terminal output will not contain Kolab welcome message
- Example output file:
...
Kolab allows you to connect your GitHub account to open and save files directly from your repositories.

### Authentication

Kolab requires `repo` access scope to read and write to your repositories. For more information, refer to the [GitHub Access Permissions](#github-access-permissions) section.
...
Kolab will automatically detect the language of the file and switch the editor's language mode accordingly. Other users' editors will also automatically switch to the same language mode.
...
Kolab supports video and voice communication between participants in a room. You can toggle your camera, microphone, and audio settings in the Video & Voice Communication panel.

Upon joining the room, Kolab will prompt you to grant permissions to access your media devices (camera, microphone, audio).
...
Kolab does not store any user data on the server except for:
...
When using Kolab, your data is encrypted in transit using HTTPS and stored securely on the server. However, it is recommended to avoid sharing sensitive information on the platform.

Kolab uses [Vercel Analytics](https://vercel.com/docs/analytics) and [Cloudflare Web Analytics](https://developers.cloudflare.com/web-analytics/) to collect anonymous usage data for improving the platform including:
...
Kolab does not store your GitHub access token. The token is stored securely in your browser's Cookies and is used only for making API requests to GitHub on your behalf.
...
You can revoke access at any time by disconnecting your GitHub account from Kolab by going to `Settings > Github Connection` and clicking `Disconnect`.
